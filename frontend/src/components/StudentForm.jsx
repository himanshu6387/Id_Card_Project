...
const StudentForm = () => {
  ...
  const fetchLinkDetails = async () => {
    try {
      const response = await axios.get(`http://localhost:5500/api/student/link/${linkId}`);
  ...
  const handleRemoveBackground = async () => {
    setRemovingBg(true);
    try {
      const response = await axios.post('http://localhost:5500/api/student/remove-background', {
        image: originalImage
      });
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `http://localhost:5500/api/student/submit/${linkId}`,
        submitData,
